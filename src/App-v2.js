...
function MovieDetails({ selectedId, onCloseMovie, onAddWatched, watched }) {
  ...
  function handleAdd() {
    ...
    // setAvgRating((avgRating) => (Number(avgRating) + Number(userRating)) / 2);
    ...
    function () {
      async function getMovieDetails() {
        try {
          ...
          const data = await res.json();
          if (data.Response === "False") throw new Error("Loading is failed!");
          // console.log(data);
          setMovie(data);
        } catch (err) {
  ...
  useEffect(
    function () {
      ...
      return function () {
        ...
        // console.log(`Clean up with ${title}`);
      };
    },
    [title]
  );
